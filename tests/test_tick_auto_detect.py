...
class EndToEndAccountingTest(unittest.TestCase):
    def test_mop_wash_and_area_dosing_fire_via_auto_detected_sensors(self) -> None:
        ...
        # The waste/dirty tank mirrors the same volumes -- the water that
        # leaves the clean tank is what fills the dirty one.
        self.assertEqual(tank_state["waste_used_ml"], 183)

    def test_waste_tank_auto_empties_on_full_to_ok_transition(self) -> None:
        """The scenario the waste-tank feature exists for: the dock error
        source reports the tank is full, then clears to "ok" -- the
        integration must treat that exactly like pressing the "Waste tank
        emptied" button, resetting waste_used_ml but leaving the clean
        tank's used_ml untouched."""
        tick, storage_mod = _load_tick()
        hass = _roborock_fixture()
        ...
        async def scenario():
            ...
            # Establish some usage on both tanks and a baseline "ok" dock state.
            hass.states._states["sensor.kitchen_robot_status"] = FakeState("cleaning")
            await tick.async_tick_water_state(hass, storage)
            state = await storage.async_get_state()
            state["tank_states"]["vacuum.kitchen_robot"]["used_ml"] = 400
            state["tank_states"]["vacuum.kitchen_robot"]["waste_used_ml"] = 2800
            await storage.async_set_tank_states(state["tank_states"])

            # Dock reports the waste tank is full.
            hass.states._states["sensor.kitchen_robot_dock_error"] = FakeState(
                "waste_water_tank_full"
            )
            await tick.async_tick_water_state(hass, storage)

            # User empties it; dock clears back to Ok.
            hass.states._states["sensor.kitchen_robot_dock_error"] = FakeState("Ok")
            await tick.async_tick_water_state(hass, storage)

            state = await storage.async_get_state()
            return state["tank_states"]["vacuum.kitchen_robot"]

        tank_state = asyncio.run(scenario())

        self.assertEqual(tank_state["waste_used_ml"], 0)
        self.assertIsNotNone(tank_state["last_waste_reset_iso"])
        # The clean tank's own counter/reset must be completely unaffected
        # by the waste-tank-specific empty event.
        self.assertEqual(tank_state["used_ml"], 400)
        self.assertIsNone(tank_state["last_reset_iso"])

    def test_custom_messages_are_honored_end_to_end(self) -> None:
        """A vacuum brand that phrases its dock errors completely
        differently from the Roborock defaults must still work, once the
        messages are customized via the config flow's edit step."""
        tick, storage_mod = _load_tick()
        hass = _roborock_fixture()
        ...
        async def scenario():
            ...
            await tick.async_ensure_auto_config(hass, storage)
            await tick.async_tick_water_state(hass, storage)  # seed a tank_state entry

            state = await storage.async_get_state()
            state["tank_states"]["vacuum.kitchen_robot"]["used_ml"] = 900
            await storage.async_set_tank_states(state["tank_states"])

            hass.states._states["sensor.kitchen_robot_dock_error"] = FakeState("TankLow")
            await tick.async_tick_water_state(hass, storage)

            hass.states._states["sensor.kitchen_robot_dock_error"] = FakeState("AllGood")
            await tick.async_tick_water_state(hass, storage)
            final_state = (await storage.async_get_state())["tank_states"]["vacuum.kitchen_robot"]
...
class DockErrorValueTest(unittest.TestCase):
    def test_reads_main_state_when_no_attribute_configured(self) -> None:
        tick, _storage = _load_tick()
        hass = _roborock_fixture()
        hass.states._states["sensor.kitchen_robot_dock_error"] = FakeState("water_empty")

        value = tick._dock_error_value(hass, {"dock_error_sensor": "sensor.kitchen_robot_dock_error"})
        self.assertEqual(value, "water_empty")

    def test_reads_configured_attribute_instead_of_state(self) -> None:
        """The scenario a docking-station entity whose main .state is
        something else entirely (e.g. "docked"), with the actual fault
        message living in an attribute -- set via the config flow's
        "Edit vacuum -> Set dock error sensor" step."""
        tick, _storage = _load_tick()
        hass = _roborock_fixture()
        hass.states._states["sensor.dock_station"] = FakeState(
            "docked", attributes={"error": "Water empty"}
        )

        value = tick._dock_error_value(
            hass,
            {"dock_error_sensor": "sensor.dock_station", "dock_error_attribute": "error"},
        )
        self.assertEqual(value, "Water empty")

    def test_missing_entity_returns_none(self) -> None:
        tick, _storage = _load_tick()
        hass = _roborock_fixture()
        self.assertIsNone(tick._dock_error_value(hass, {"dock_error_sensor": "sensor.missing"}))

    def test_no_configured_sensor_returns_none(self) -> None:
        tick, _storage = _load_tick()
        hass = _roborock_fixture()
        self.assertIsNone(tick._dock_error_value(hass, {}))


class MatchesDockMessageTest(unittest.TestCase):
    def test_matches_default_empty_message(self) -> None:
        tick, _storage = _load_tick()
        self.assertTrue(
            tick.matches_dock_message("water_empty", tick.DEFAULT_DOCK_EMPTY_MESSAGE)
        )

    def test_matches_human_readable_variant(self) -> None:
        """The exact case reported by a user: an attribute value of
        "Water empty" (capitalized, space instead of underscore) must be
        recognized the same as the raw "water_empty" enum value, without
        the user needing to type an exact-case match string."""
        tick, _storage = _load_tick()
        self.assertTrue(tick.matches_dock_message("Water empty", "water_empty"))
        self.assertTrue(tick.matches_dock_message("water_empty", "Water empty"))

    def test_custom_message_is_honored(self) -> None:
        """A fully custom message (a different vacuum brand's wording)
        must match itself and nothing else -- this is the whole point of
        making the messages configurable."""
        tick, _storage = _load_tick()
        self.assertTrue(
            tick.matches_dock_message("CleanWaterTankEmpty", "CleanWaterTankEmpty")
        )
        self.assertFalse(
            tick.matches_dock_message("CleanWaterTankEmpty", tick.DEFAULT_DOCK_EMPTY_MESSAGE)
        )

    def test_non_matching_value_does_not_match(self) -> None:
        tick, _storage = _load_tick()
        self.assertFalse(tick.matches_dock_message("ok", tick.DEFAULT_DOCK_EMPTY_MESSAGE))
        self.assertFalse(tick.matches_dock_message("duct_blockage", tick.DEFAULT_DOCK_EMPTY_MESSAGE))

    def test_none_and_empty_do_not_match(self) -> None:
        tick, _storage = _load_tick()
        self.assertFalse(tick.matches_dock_message(None, tick.DEFAULT_DOCK_EMPTY_MESSAGE))
        self.assertFalse(tick.matches_dock_message("", tick.DEFAULT_DOCK_EMPTY_MESSAGE))
        self.assertFalse(tick.matches_dock_message("water_empty", None))
        self.assertFalse(tick.matches_dock_message("water_empty", ""))


class DockErrorResetEndToEndTest(unittest.TestCase):
    def test_reset_fires_from_attribute_with_human_readable_text(self) -> None:
        """Full accounting-loop proof: a dock error sourced from an
        attribute, with human-readable text, still triggers the
        water-empty-cleared auto-reset -- not just the isolated helpers."""
        tick, storage_mod = _load_tick()
        hass = _roborock_fixture()
        hass.states._states["sensor.dock_station"] = FakeState(
            "docked", attributes={"error": "Water empty"}
        ...
        async def scenario():
            ...
            # Tick 1: observes the water_empty error, establishes baseline.
            await tick.async_tick_water_state(hass, storage)
            state = await storage.async_get_state()
            used_before = state["tank_states"]["vacuum.kitchen_robot"].get("used_ml", 0)

            # Give it some usage to make sure the reset actually zeroes something.
            state["tank_states"]["vacuum.kitchen_robot"]["used_ml"] = 500
            await storage.async_set_tank_states(state["tank_states"])

            # Tick 2: error clears (tank refilled at the dock) -> auto-reset.
            hass.states._states["sensor.dock_station"] = FakeState(
                "docked", attributes={"error": "Ok"}
            )
            await tick.async_tick_water_state(hass, storage)
            state = await storage.async_get_state()
            return used_before, state["tank_states"]["vacuum.kitchen_robot"]["used_ml"]

        used_before, used_after = asyncio.run(scenario())
