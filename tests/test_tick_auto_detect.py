...
class EnsureAutoConfigTest(unittest.TestCase):
    def test_persists_capacity_and_companions_for_new_device(self) -> None:
...
class EndToEndAccountingTest(unittest.TestCase):
    def test_mop_wash_and_area_dosing_fire_via_auto_detected_sensors(self) -> None:
        """The actual scenario this whole feature exists for: a zero-config
        auto-discovered vacuum, with no manual Settings-tab configuration
        at all, must still accumulate used_ml from real cleaning activity."""
        tick, storage_mod = _load_tick()
        hass = _roborock_fixture()
