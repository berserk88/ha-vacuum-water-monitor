"""Store-backed persistence for Vacuum water level."""
...
class VacuumWaterStorage:
    ...
    async def async_reset_waste_tank(
        self, vacuum_entity: str, when_iso: str, when_ts: int
    ) -> dict[str, Any]:
        """Reset the dirty/waste tank counter (the "Emptied" button) and
        return the new tank state. Independent of async_reset_tank -- the
        clean and waste tanks are refilled/emptied on their own schedules."""
        async with self._lock:
            data = await self._ensure_loaded_locked()
            state = self.default_tank_state()
            state.update(data["tank_states"].get(vacuum_entity) or {})
            state["waste_used_ml"] = 0
            state["last_waste_reset_iso"] = when_iso
            state["last_waste_reset_ts"] = when_ts
            data["tank_states"][vacuum_entity] = state
            await self._store.async_save(data)
            return deepcopy(state)

    async def async_reset_prediction_models(
        self, vacuum_entity: str
    ) -> dict[str, Any]:
        """Clear both adaptive calibration models (water and waste) back
        to their unlearned defaults -- the "Clear prediction model"
        button. Does NOT touch the current tank levels/counters or the
        mop mode/intensity latches; only the learned per-intensity
        correction factors and calibration history are reset."""
        async with self._lock:
            data = await self._ensure_loaded_locked()
            state = self.default_tank_state()
            state.update(data["tank_states"].get(vacuum_entity) or {})
            state["water_model"] = self.default_prediction_model()
            state["waste_model"] = self.default_prediction_model()
            data["tank_states"][vacuum_entity] = state
            await self._store.async_save(data)
            return deepcopy(state)
    ...
    def default_tank_state() -> dict[str, Any]:
        """Return the v4-compatible tank state shape, extended with the
        dirty/waste-tank counters, the mop mode/intensity latches (see
        tick.py module docstring), and the two adaptive calibration
        models (water/waste)."""
        return {
            "used_ml": 0,
            "last_reset_iso": None,
            ...
            "last_reset_ts": 0,
            "waste_used_ml": 0,
            "last_waste_reset_iso": None,
            "last_waste_reset_ts": 0,
            "latched_mop_mode": None,
            "latched_mop_intensity": None,
            "water_model": VacuumWaterStorage.default_prediction_model(),
            "waste_model": VacuumWaterStorage.default_prediction_model(),
        }

    @staticmethod
    def default_prediction_model() -> dict[str, Any]:
        """Return the default (unlearned) shape of one adaptive
        calibration model. `correction` is a per-intensity multiplier
        applied on top of the static default dosing formula, empty
        initially (meaning 1.0x / no correction everywhere) and adjusted
        over time — see tick.py::_calibrate_model."""
        return {
            "correction": {},
            "pending_ml_by_intensity": {},
            "cycles_observed": 0,
            "last_calibrated_iso": None,
        }
