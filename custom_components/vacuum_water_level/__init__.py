"""Vacuum water level integration entry points."""
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    await _async_migrate_legacy_storage(hass)
    ...
    _LOGGER.debug("Vacuum water level set up (entry_id=%s)", entry.entry_id)
    return True


async def _async_migrate_legacy_storage(hass: HomeAssistant) -> None:
    """One-time migration for anyone coming from the original
    `ha_vacuum_water_monitor` domain this integration was renamed from.

    If the legacy domain's storage file exists and this integration's own
    storage file doesn't exist yet, copy it forward — otherwise every
    tracked vacuum and its tank history would silently vanish on first
    setup after the rename. Never overwrites data that already exists
    under the new domain.
    """
    from homeassistant.helpers.storage import Store

    new_store: Store[dict[str, Any]] = Store(hass, STORAGE_VERSION, DOMAIN)
    if await new_store.async_load() is not None:
        return  # already has its own data; never overwrite

    legacy_store: Store[dict[str, Any]] = Store(hass, STORAGE_VERSION, LEGACY_DOMAIN)
    ...
    _LOGGER.info(
        "Migrated storage from the legacy '%s' domain to '%s' (integration renamed)",
        LEGACY_DOMAIN,
        DOMAIN,
    )
