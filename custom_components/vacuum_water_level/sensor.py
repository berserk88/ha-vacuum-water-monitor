"""Sensor entities for Vacuum water level."""
...
class VacuumSensorManager:
    ...
    async def async_sync_devices(self) -> None:
        """Add sensor entities for any newly tracked vacuum."""
        ...
        # discovered is passed only so an already-tracked device without a
        # stored name gets backfilled with its HA friendly name -- it can no
        # longer create new devices (tracking is opt-in via the config flow).
        ...
        for device in devices:
            ...
            for sensor_cls in (
                ...
                WaterUsedSensor,
                LastRefillSensor,
                WasteTankLevelSensor,
                WasteWaterCollectedSensor,
                LastEmptiedSensor,
            ):
...
def _model_attributes(model: dict[str, Any] | None) -> dict[str, Any]:
    """Extract display-friendly attributes from a stored adaptive
    calibration model (see tick.py::_calibrate_model) for exposing on a
    sensor's extra_state_attributes -- lets the person see how the
    prediction has learned over time instead of it being a black box."""
    model = model if isinstance(model, dict) else {}
    correction = model.get("correction")
    return {
        "prediction_cycles_observed": model.get("cycles_observed", 0),
        "prediction_correction_factors": correction if isinstance(correction, dict) else {},
        "prediction_last_calibrated": model.get("last_calibrated_iso"),
    }
...
class WaterUsedSensor(VacuumStoreSensor):
    ...
    async def async_update(self) -> None:
        ...
        self._attr_extra_state_attributes = {
            ...
            "last_refill": tank_state.get("last_reset_iso"),
            **_model_attributes(tank_state.get("water_model")),
        }
...
class WasteTankLevelSensor(VacuumStoreSensor):
    """Estimated dirty/waste water tank fill percentage."""

    sensor_key = "waste_tank_level"
    sensor_name = "Waste tank level"
    _attr_native_unit_of_measurement = PERCENTAGE
    _attr_state_class = SensorStateClass.MEASUREMENT

    async def async_update(self) -> None:
        """Update waste tank fill level from Store."""
        settings, tank_state = await self._store_context()
        estimate = estimate_waste_state(self._device, tank_state, settings)
        self._attr_native_value = estimate["full_percent"]
        self._attr_extra_state_attributes = {
            "vacuum_entity": self.vacuum_entity,
            "source": estimate["source"],
            "total_ml": estimate["total_ml"],
            "collected_ml": estimate["collected_ml"],
            "last_emptied": tank_state.get("last_waste_reset_iso"),
        }


class WasteWaterCollectedSensor(VacuumStoreSensor):
    """Waste water collected since the tank was last emptied."""

    sensor_key = "waste_water_collected"
    sensor_name = "Waste water collected"
    _attr_native_unit_of_measurement = WATER_VOLUME_UNIT
    _attr_state_class = SensorStateClass.MEASUREMENT

    async def async_update(self) -> None:
        """Update collected waste water from Store."""
        settings, tank_state = await self._store_context()
        estimate = estimate_waste_state(self._device, tank_state, settings)
        self._attr_native_value = estimate["collected_ml"]
        self._attr_extra_state_attributes = {
            "vacuum_entity": self.vacuum_entity,
            "source": estimate["source"],
            "total_ml": estimate["total_ml"],
            ...
            "last_emptied": tank_state.get("last_waste_reset_iso"),
            **_model_attributes(tank_state.get("waste_model")),
        }


class LastEmptiedSensor(VacuumStoreSensor):
    """Timestamp of the last waste-tank-emptied reset."""

    sensor_key = "last_emptied"
    sensor_name = "Last emptied"
    _attr_device_class = SensorDeviceClass.TIMESTAMP
    _attr_entity_category = EntityCategory.DIAGNOSTIC

    async def async_update(self) -> None:
        """Update last-emptied timestamp from Store."""
        _settings, tank_state = await self._store_context()
        self._attr_native_value = parse_waste_reset_datetime(tank_state)
        self._attr_extra_state_attributes = {
            "vacuum_entity": self.vacuum_entity,
            "last_waste_reset_iso": tank_state.get("last_waste_reset_iso"),
            "last_waste_reset_ts": tank_state.get("last_waste_reset_ts"),
        }
