"""Pure Store-derived calculations for Vacuum water level sensors."""
...
def estimate_waste_state(
    device: dict[str, Any] | None,
    tank_state: dict[str, Any] | None,
    settings: dict[str, Any] | None = None,
) -> dict[str, Any]:
    """Estimate the dirty/waste water tank's fill level from stored usage
    and stored capacity. Mirrors estimate_water_state, but tracks how FULL
    the tank is rather than how much is remaining -- the waste tank fills
    up as the vacuum cleans (the inverse of the clean tank emptying)."""
    device = device if isinstance(device, dict) else {}
    tank_state = tank_state if isinstance(tank_state, dict) else {}
    settings = settings if isinstance(settings, dict) else {}

    collected_ml = max(0, _number(tank_state.get("waste_used_ml"), 0))
    total_ml = _waste_capacity_ml(device, settings)
    if total_ml is None:
        return {
            "source": "unknown_capacity",
            "total_ml": None,
            "collected_ml": _format_number(collected_ml),
            "full_percent": None,
        }

    percent = _clamp((collected_ml / total_ml) * 100, 0, 100)
    return {
        "source": "stored_estimate",
        "total_ml": _format_number(total_ml),
        "collected_ml": _format_number(collected_ml),
        "full_percent": _format_number(round(percent, 1)),
    }


def _parse_stored_datetime(
    tank_state: dict[str, Any], iso_key: str, ts_key: str
) -> datetime | None:
    raw_iso = tank_state.get(iso_key)
    ...
    raw_ts = _optional_number(tank_state.get(ts_key))
...
def parse_refill_datetime(tank_state: dict[str, Any] | None) -> datetime | None:
    ...
    tank_state = tank_state if isinstance(tank_state, dict) else {}
    return _parse_stored_datetime(tank_state, "last_reset_iso", "last_reset_ts")


def parse_waste_reset_datetime(tank_state: dict[str, Any] | None) -> datetime | None:
    """Parse the Store waste-tank-emptied timestamp as an aware UTC
    datetime. Mirrors parse_refill_datetime for the dirty/waste tank."""
    tank_state = tank_state if isinstance(tank_state, dict) else {}
    return _parse_stored_datetime(
        tank_state, "last_waste_reset_iso", "last_waste_reset_ts"
    )
...
def _waste_capacity_ml(
    device: dict[str, Any], settings: dict[str, Any]
) -> float | None:
    """Waste/dirty tank capacity: an explicit waste_total_ml (set via the
    config flow's dock-error edit step) if given, otherwise falls back to
    the same capacity as the clean tank -- a reasonable default since many
    vacuums use similarly-sized clean/dirty tanks, and it's overridable
    per-vacuum when it isn't accurate."""
    direct = _optional_number(device.get("waste_total_ml"))
    if direct and direct > 0:
        return direct
    return _water_capacity_ml(device, settings)
